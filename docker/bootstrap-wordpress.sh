...
readonly HTACCESS_TEMPLATE="/usr/local/share/wordpress-stack/.htaccess.template"
...
install_or_convert_network() {
    ...
    if ! wp_cli core is-installed >/dev/null 2>&1; then
        log "Installo WordPress Multisite in modalità sottocartelle."
        wp_cli "${install_args[@]}"
    elif ! wp_cli core is-installed --network >/dev/null 2>&1; then
        log "Converto l'installazione esistente in Multisite sottocartelle."
        wp_cli core multisite-convert --title="${WORDPRESS_TITLE}"
    else
        log "WordPress Multisite è già installato: nessuna reinstallazione."
    fi
}
...
configure_wordpress() {
    ...
    set_raw_constant SUBDOMAIN_INSTALL false
...
configure_plugins() {
    ...
    if [[ ${install_plugins} == true ]]; then
        ...
        wp_cli plugin activate litespeed-cache --network --quiet
        ...
        wp_cli plugin activate redis-cache --network --quiet
    else
        log "Installazione automatica dei plugin disabilitata."
    fi

    if [[ ${enable_redis} == true && ${install_plugins} == true ]]; then
        if ! wp_cli plugin is-active redis-cache --network; then
            warn "Redis Object Cache non è network-attivo; salto l'abilitazione del drop-in."
            return 0
        fi
        ...
        wp_cli plugin deactivate redis-cache --network --quiet >/dev/null 2>&1 || true
...
main() {
    ...
    local debug skip_email install_plugins enable_redis disable_cron host canonical_url redis_available
    ...
    log "Dominio normalizzato: ${host}; URL canonico: ${canonical_url}."
    ...
    install_or_convert_network "${canonical_url}" "${skip_email}"
    configure_wordpress "${host}" "${canonical_url}" "${debug}" "${disable_cron}" "${install_plugins}"
    configure_plugins "${install_plugins}" "${enable_redis}"
    manage_htaccess
    ...
    log "Bootstrap completato; Network Admin: ${canonical_url}/wp-admin/network/."
}
